...
import slugify, { slugOptions } from './slugify';
...
export const search = async ({
  ...
  if (searchParam) {
    ...
    searchArray = searchArray.map((value) => slugify(value, slugOptions));
    ...
    query = query.where('tags', 'array-contains-any', searchArray.slice(0, 10));
  }
...
export const getTrending = ({ type, startAfter, limit } = {}) => search({
  type, startAfter, limit, orderBy: 'views', orderDir: 'desc',
});

export const getNew = ({ type, startAfter, limit } = {}) => search({
